...
const TitlesYouOwn = (props) => {
  ...
  const getData = ()=>{
    setLoading(true)
        ...
        doConsole(" I request @ "+urls.API+"get_games");
        doConsole(body_data);
        fetch(urls.API + 'get_games', {
            method: 'POST',
            headers: {
  ...
  const goSignup = ()=>{
    ...
    for(var i = 0; i<=interestsArray.length-1; i++)
    {
        ...
        doConsole(" I request @ "+urls.API+"update_games");
        doConsole(body_data);
        fetch(urls.API + 'update_games', {
            method: 'POST',
            headers: {
                Accept: 'application/json',
                'Content-Type': 'application/json',
        ...
        <View style={{ flex: 1, }}>
            ...
            >
                ...
                <View style={{ flexDirection: 'row', marginTop: 50 }}>
                    <View style={{ width: 223 - 43, height: 39, borderWidth: 1, borderLeftWidth: 0, marginLeft: -5, borderColor: '#FFFFFF', borderRadius: 9, justifyContent: 'center', alignItems: 'center' }}>
                        <Text style={{ fontSize: 18, fontFamily: 'PMe', color: '#FFFFFF' }}>Games I Like</Text>
                    </View>
